...
class HistoryService {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (data.analyses && !(data.analyses instanceof Map)) {
          data.analyses = new Map(
            Object.entries(data.analyses).map(([k, v]) => [parseInt(k, 10), v as AnalysisResult]),
          );
        }
  ...
  async updateHistoryAnalyses(
    ...
    await this.saveToHistory(owner, repo, entry.data.issues, mergedAnalyses);
  }

  async getAllHistory(): Promise<
    Array<{ key: string; metadata: HistoryMetadata; fetchedAt: number }>
  > {
    await this.init();
